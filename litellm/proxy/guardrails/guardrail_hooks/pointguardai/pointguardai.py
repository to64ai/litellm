    ...
    def _extract_violations(self, response_data: dict, input_blocked: bool, output_blocked: bool) -> List[dict]:
        ...
        def extract_from_content(content_items: List[dict]) -> List[dict]:
            ...
            for content_item in content_items:
                ...
                # Extract AI violations (name = display e.g. "Jailbreak"; type = threat_type e.g. "JAILBREAK", "PROMPT_INJECTION")
                ai_violations = content_item.get("aiViolations", [])
                for ai in ai_violations:
                    threat_type = ai.get("type") or ai.get("threatType") or "UNKNOWN"
                    name = ai.get("name", "Unknown")
                    verbose_proxy_logger.debug(
                        "PointGuardAI aiViolation: name=%s, type=%s, aiThreatCategoryId=%s",
                        name, threat_type, ai.get("aiThreatCategoryId"),
                    )
                    all_violations.append({
                        "type": "AI_THREAT",
                        "name": name,
    ...
    def _create_violation_details(self, violations: List[dict]) -> List[dict]:
        ...
        for violation in violations:
            ...
            if violation_type == "DLP":
                ...
                # AI threat: surface threat_type (JAILBREAK, PROMPT_INJECTION, etc.) as primary category
                ...
                violation_details.append({
                    ...
                    "name": name,
                    "threat_type": threat_type,
                    "category": threat_type,  # primary label for clients: JAILBREAK vs PROMPT_INJECTION etc.
                    "action": violation.get("action", "UNKNOWN"),
